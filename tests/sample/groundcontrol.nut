...
scenes:
    $prelaunch ('EXT. a rocket stands ready to launch'):
        ...
        - callsign:
            as @groundControl:
                tell  @majorTom 'Ground Control to Major Tom'
        ...
        - launch_comms:
            ...
            delay 4s
            as @majorTom:
                tell @groundControl `Commencing countdown, engines on `
            delay 3s
            as @groundControl:
                tell @majorTom `Check ignition and may God's love be with you `
...
And the papers want to know whose shirts you wear`
    ...
    $spacewalk:
        ...
        ('EXT. Tom is in space suit looking out the window we see him through the window' ):
            as @groundControl:
                tell  @majorTom `Now it's time to leave the capsule if you dare`
        
        ('INT. Tom is in space suit looking out open door'):
            as @majorTom:
                tell @groundControl "This is Major Tom to Ground Control"
...
`
        ...
        (`INT. Control Center`):
            as @groundControl:
                tell @majorTom `Ground Control to Major Tom`
            as @groundControl:
                tell @majorTom `Your circuit's dead, there's something wrong`
            for i in range(3):
                as @groundControl:
                    tell @majorTom `Can you hear me, Major Tom?`
                delay 500ms
            @groundControl:
                `Can you...`
