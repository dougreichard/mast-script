roles:
    ...
    #role4 "double quotes"
cast:
    @cast1 #role1 `test cast`
    @cast2('Alias name') #role2 `Test alias`
    @cast3('Alias name') `Test no role`

    @cast4  [#role1 #role2] `test cast multi role`

    @cast5('Alias name')  [#role1 #role2] `Test alias`

    fred ('alias') 'desc'

    // test cast using keyword
    @trigger ('Test') #role1 ` `
